...
func main() {
	...
	flag.UintVar(&conf.Listen, "listen", conf.Listen, "TCP port for HTTP listener of this proxy")
	...
	if conf.Listen == 0 {
		fmt.Fprintln(os.Stderr, "Listen port must be specified (non-zero)")
		flag.Usage()
		os.Exit(1)
	}
	...
	// "/usr/local/garagedoor/var/db"
	config := hc.Config{Pin: conf.PIN, StoragePath: conf.StoragePath}
